...
class LaporanController extends Controller
{
    ...
    public function jpenjualan()
    {
        $data =  new \stdClass();
        $data->total = 0;
        $data->list = DB::table('kas')->where('kas_ket', 'jual')->join('jual', 'jual.jual_id', '=', 'kas_id_value')->orderby('kas_tgl', 'asc')->get();
        if ($data->list) {
            foreach ($data->list as $item) {
    ...
    public function lpembelian(Request $request)
    {
        ...
        $data->total = 0;
        $data->list = DB::table('kas')->where('kas_ket', 'beli')->whereBetween('kas_tgl', [$data->startdate, $data->enddate])->join('beli', 'beli_id', '=', 'kas_id_value')->join('supplier', 'supplier.supplier_id', '=', 'beli.supplier_id')->orderby('kas_tgl', 'asc')->get();
        //        dd($data->list);
        if ($data->list) {
            foreach ($data->list as $item) {
                $data->total = $data->total + $item->kas_kredit;
    ...
    public function lpenerimaankas(Request $request)
    {
        ...
        $data->startdate = ($request->startdate) ? $request->startdate : Carbon::now()->startOfMonth()->format('Y-m-d');
        $data->total = 0;
        $data->list = DB::table('kas')->wherein('kas_ket', ['jual', 'retur'])->whereBetween('kas_tgl', [$data->startdate, $data->enddate])->orderby('kas_tgl', 'asc')->get();
        //        dd($data->list);
        if ($data->list) {
            foreach ($data->list as $item) {
                $data->total = $data->total + $item->kas_debet;
